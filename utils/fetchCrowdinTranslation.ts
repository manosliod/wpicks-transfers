...
export const fetchCrowdinTranslation = async (
  ...
  try {
    ...
    const file = filesRes.data.data.find(
      (f: any) => f.data.name === `${namespace}.json`
    );

    if (!file) {
      console.warn(
        `[Crowdin] No translation file found for namespace: ${namespace}`
      );
      return;
    }
